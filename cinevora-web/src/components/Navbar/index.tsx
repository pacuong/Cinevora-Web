...
const NavBarComponent = ({ navList }: NavbarProps) => {
  return (
    <nav className="bg-black-40 py-8 md:pt-[25px] md:pb-[22px] lg:pb-11 lg:mr-auto lg:ml-[170px]">
        ...
        {navList.map((item, index) => (
          <li className="" key={index}>
            {item.type === "dropdown" ? (
              ...
              <Link
                className="text-white-100 active:text-orange-90 lg:mx-9 lg:font-bold hover:text-orange-90 font-saira md:text-base lg:text-[20px] lg:hover:border-b-2 lg:active:border-b-2 lg:active:pb-5 lg:hover:pb-5 uppercase"
                href={item.href}
              >
                {item.content}
              </Link>
